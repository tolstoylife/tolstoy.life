...
The [[messages-count-tokens|token counting]] endpoint accepts the same structured list of inputs for creating a message, including support for system prompts, [[overview|tools]], [[vision|images]], and [[pdf-support|PDFs]]. The response contains the total number of input tokens.
...
All [[overview|active models]] support token counting.
...
> [!note]
> [[overview#server-tools|Server tool]] token counts only apply to the first sampling call.
...
> [!note]
> See [[extended-thinking#how-context-window-is-calculated-with-extended-thinking|here]] for more details about how the context window is calculated with extended thinking
>
...
> [!note]
> Token counting supports PDFs with the same [[pdf-support#pdf-support-limitations|limitations]] as the Messages API.
...
Token counting is **free to use** but subject to requests per minute rate limits based on your [[rate-limits#rate-limits|usage tier]]. If you need higher limits, contact sales through the [Claude Console](https://console.anthropic.com/settings/limits).
...
> [!info]- Does token counting use prompt caching?
...
**Source:** [Official Documentation](https://docs.claude.com/en/docs/build-with-claude/token-counting)
