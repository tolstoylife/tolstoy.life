...
* Claude Sonnet 3.7 (`claude-3-7-sonnet-20250219`) ([[model-deprecations|deprecated]])
...
For more information about the response format of extended thinking, see the [[messages|Messages API Reference]].
...
For more documention on streaming via the Messages API, see [[streaming|Streaming Messages]].
...
Extended thinking can be used alongside [[overview|tool use]], allowing Claude to reason through tool selection and results processing.
...
> [!info]- Example: Passing thinking blocks with tool results
...
2. **Context maintenance**: While tool results appear as user messages in the API structure, they're part of a continuous reasoning flow. Preserving thinking blocks maintains this conceptual flow across multiple API calls. For more information on context management, see our [[context-windows|guide on context windows]].
...
To enable interleaved thinking, add [[beta-headers|the beta header]] `interleaved-thinking-2025-05-14` to your API request.

Here are some important considerations for interleaved thinking:

* With interleaved thinking, the `budget_tokens` can exceed the `max_tokens` parameter, as it represents the total budget across all thinking blocks within one assistant turn.
* Interleaved thinking is only supported for [[overview|tools used via the Messages API]].
...
* On 3rd-party platforms (e.g., [[claude-on-amazon-bedrock|Amazon Bedrock]] and [[claude-on-vertex-ai|Vertex AI]]), if you pass `interleaved-thinking-2025-05-14` to any model aside from Claude Opus 4.1, Opus 4, or Sonnet 4, your request will fail.

> [!info]- Tool use without interleaved thinking
> ```python Python theme={null}
>       import anthropic
>
>       client = anthropic.Anthropic()
...
>     3. When tool results are returned, Claude immediately provides a response without additional thinking
  ...
  > [!info]- Tool use with interleaved thinking
> ```python Python theme={null}
>       import anthropic
>
>       client = anthropic.Anthropic()
...
[[prompt-caching|Prompt caching]] with thinking has several important considerations:

> [!tip]
> Extended thinking tasks often take longer than 5 minutes to complete. Consider using the [[prompt-caching#1-hour-cache-duration|1-hour cache duration]] to maintain cache hits across longer thinking sessions and multi-step workflows.
...
> [!info]- System prompt caching (preserved when thinking changes)
> ```python Python theme={null}
...
>       ```
  ...
  > [!info]- Messages caching (invalidated when thinking changes)
> ```python Python theme={null}
>       from anthropic import Anthropic
>       import requests
>       from bs4 import BeautifulSoup
...
> [!note]
> You can read through our [[context-windows|guide on context windows]] for a more thorough deep dive.
...
We recommend using the [[token-counting|token counting API]] to get accurate token counts for your specific use case, especially when working with multi-turn conversations that include thinking.
...
* Potentially use the [[token-counting|token counting endpoints]] more frequently
...
* `signature` values are compatible across platforms (Claude APIs, [[claude-on-amazon-bedrock|Amazon Bedrock]], and [[claude-on-vertex-ai|Vertex AI]]). Values generated on one platform will be compatible with another.
...
> [!info]- Example: Working with redacted thinking blocks
...
* **Large budgets:** For thinking budgets above 32k, we recommend using [[batch-processing|batch processing]] to avoid networking issues. Requests pushing the model to think above 32k tokens causes long running requests that might run up against system timeouts and open connection limits.
...
* Thinking isn't compatible with `temperature` or `top_k` modifications as well as [[implement-tool-use#forcing-tool-use|forced tool use]].
...
* **Prompt engineering:** Review our [[extended-thinking-tips|extended thinking prompting tips]] if you want to maximize Claude's thinking capabilities.

## Next steps


> [!info] Try the extended thinking cookbook
> Explore practical examples of thinking in our cookbook.

  > [!info] Extended thinking prompting tips
> Learn prompt engineering best practices for extended thinking.


---

**Source:** [Official Documentation](https://docs.claude.com/en/docs/build-with-claude/extended-thinking)
