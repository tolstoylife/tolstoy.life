...
These limits apply to both Standard and Priority Tier usage. For more information about Priority Tier, which offers enhanced service levels in exchange for committed spend, see [[service-tiers|Service Tiers]].
...
If you exceed any of the rate limits you will get a [[errors|429 error]] describing which rate limit was exceeded, along with a `retry-after` header indicating how long to wait.
...
>
>   For rate limit purposes on most models, only `input_tokens` + `cache_creation_input_tokens` count toward your ITPM limit, making [[prompt-caching|prompt caching]] an effective way to increase your effective throughput.
...
>
>   For all models without the † marker, cached input tokens do not count towards rate limits and are billed at a reduced rate (10% of base input token price). This means you can achieve significantly higher effective throughput by using [[prompt-caching|prompt caching]].

> [!tip]
> **Maximize your rate limits with prompt caching**
>
>   To get the most out of your rate limits, use [[prompt-caching|prompt caching]] for repeated content like:
>
...
| Model                                                                      | Maximum requests per minute (RPM) | Maximum input tokens per minute (ITPM) | Maximum output tokens per minute (OTPM) |
    ...
    | Claude Sonnet 3.7 ([[model-deprecations|deprecated]]) | 50                                | 20,000                                 | 8,000                                   |
    ...
    | Claude Opus 3 ([[model-deprecations|deprecated]])     | 50                                | 20,000<sup>†</sup>                     | 4,000                                   |
...
| Model                                                                      | Maximum requests per minute (RPM) | Maximum input tokens per minute (ITPM) | Maximum output tokens per minute (OTPM) |
    ...
    | Claude Sonnet 3.7 ([[model-deprecations|deprecated]]) | 1,000                             | 40,000                                 | 16,000                                  |
    ...
    | Claude Opus 3 ([[model-deprecations|deprecated]])     | 1,000                             | 40,000<sup>†</sup>                     | 8,000                                   |
...
| Model                                                                      | Maximum requests per minute (RPM) | Maximum input tokens per minute (ITPM) | Maximum output tokens per minute (OTPM) |
    ...
    | Claude Sonnet 3.7 ([[model-deprecations|deprecated]]) | 2,000                             | 80,000                                 | 32,000                                  |
    ...
    | Claude Opus 3 ([[model-deprecations|deprecated]])     | 2,000                             | 80,000<sup>†</sup>                     | 16,000                                  |
...
| Model                                                                      | Maximum requests per minute (RPM) | Maximum input tokens per minute (ITPM) | Maximum output tokens per minute (OTPM) |
    ...
    | Claude Sonnet 3.7 ([[model-deprecations|deprecated]]) | 4,000                             | 200,000                                | 80,000                                  |
    ...
    | Claude Opus 3 ([[model-deprecations|deprecated]])     | 4,000                             | 400,000<sup>†</sup>                    | 80,000                                  |
...
When using Claude Sonnet 4 and Sonnet 4.5 with the [[context-windows#1m-token-context-window|1M token context window enabled]], the following dedicated rate limits apply to requests exceeding 200K tokens.
...
> [!tip]
> To get the most out of the 1M token context window with rate limits, use [[prompt-caching|prompt caching]].
...
**Source:** [Official Documentation](https://docs.claude.com/en/api/rate-limits)
