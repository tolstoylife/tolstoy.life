...
category: {page['category']}"""
    ...
    def find_related_pages(self, page: Dict) -> List[str]:
        """Find related pages based on category, subcategory, and content. Returns filenames."""
        related = []
    ...
    def convert_content_to_obsidian(self, content: str, page: Dict) -> str:
        ...
        for other_page in self.pages:
            ...
            # Match [Text](url) format - try both with and without .md
            url_pattern = re.escape(other_page['url'])

            # Replace with wikilink using filename and display text
            content = re.sub(
                rf'\[([^\]]+)\]\({url_pattern}(?:\.md)?\)',
...
"""
    ...
    def build(self):
        ...
        print(f"📊 Stats: {success_count}/{len(self.pages)} pages created, {len(self.categories)} categories")
